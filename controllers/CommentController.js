...
db.get = (id, page) => {
    return new Promise((resolve, reject) => {
        let query = `select id, a.avatar, c.username, endpoint, id_root, content, files, to_char(time, 'DD-MM-YYYY hh:mm:ss') as time from "Comment" c, "Account" a where (id = $1 or id_root = $1) and c.username = a.username order by id desc`
...
db.list = (endpoint, page) => {
    return new Promise((resolve, reject) => {
        ...
        from "Comment" c, "Account" a where endpoint = $1 and id_root = id and c.username = a.username order by id desc`
        query += ' limit ' + constants.limit_element + ' offset ' + (constants.limit_element * (page - 1))
        let params = [endpoint]
        conn.query(query, params, (err, res) => {
            if(err) return reject(err)
